            ...
            if self.cache_tag:
                ...
                for image in image_list:
                    if image['Created'] > latest_created_time and image['RepoTags']:
                        latest_created_time = image['Created']
                        latest_image = image
                if latest_image:
                    latest_image_url = latest_image['RepoTags'][0]
                    latest_image_tag = latest_image_url[len(self.repository)+1:]
                    self.client.pull(repository=self.repository, tag=latest_image_tag)
                    cache_from = [latest_image_url]
                    response.append(f"Setting cache to: {cache_from}")
                else:
                    response.append(f"Unable to find a cache tag")
